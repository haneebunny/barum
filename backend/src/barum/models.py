...
class ExportReadinessStatus(str, Enum):
    """미국 수출 준비도 항목의 상태. 법적 허가/금지 판정이 아니다."""
...
class ExportReadinessCategory(str, Enum):
    """MVP가 한 번에 보여주는 미국 수출 준비도 7개 카테고리."""
...
class ExportProfile(BaseModel):
    """다음 제품에서도 재사용할 제조·수출 프로필 스냅샷.

    로그인 없는 MVP에서 프론트가 보낸 값을 그대로 보존한다. 선언값을 FDA 실시간
    상태로 승격하지 않기 위해 준비도 판정기는 별도 검증 상태를 사용한다.
    """

    model_config = ConfigDict(extra="allow", populate_by_name=True)

    legal_manufacturer: Any | None = None
    manufacturer_name: Any | None = None
    manufacturing_site: Any | None = None
    manufacturing_site_address: Any | None = None
    contract_manufacturer: Any | None = None
    us_agent_name: Any | None = None
    us_agent_contact: Any | None = None
    importer_name: Any | None = None
    importer_contact: Any | None = None
    fda_establishment_registration: Any | None = None
    fda_establishment_registration_number: Any | None = None
    registration_status: Any | None = None
    registration_renewal_date: Any | None = None
    cgmp_ready: bool | None = None
    cgmp_readiness: Any | None = None
    drug_listing_status: Any | None = None
    ndc_or_listing_number: Any | None = None


class ExportProduct(BaseModel):
    """제품 단위 준비도 입력 및 리포트 스냅샷."""

    model_config = ConfigDict(extra="allow", populate_by_name=True)

    intended_use: str | None = None
    spf_value: float | None = None
    spf_displayed: bool | None = None
    broad_spectrum: bool | None = None
    water_resistant: bool | None = None
    water_resistance_minutes: int | None = None
    spf_test_report: bool | None = None
    broad_spectrum_test_report: bool | None = None
    water_resistance_test_report: bool | None = None
    spf_test_available: bool | None = None
    broad_spectrum_test_available: bool | None = None
    water_resistance_test_available: bool | None = None
    drug_facts_ready: bool | None = None
    us_label_ready: bool | None = None
    claims_reviewed: bool | None = None
    drug_listing_ready: bool | None = None
    ingredient_amounts: Any | None = None
    bemotrizinol_evidence_provided: bool | None = None
    bemotrizinol_confirmed_ineligible: bool | None = None

    @model_validator(mode="before")
    @classmethod
    def normalize_legacy_field_names(cls, value: Any) -> Any:
        """프론트 계약이 확장될 때 흔히 쓰이는 동의어를 MVP 필드로 수렴한다."""
        if not isinstance(value, dict):
            return value
        value = dict(value)
        aliases = {
            "spf_test_available": "spf_test_report",
            "broad_spectrum_test_available": "broad_spectrum_test_report",
            "water_resistance_test_available": "water_resistance_test_report",
            "water_resistant_minutes": "water_resistance_minutes",
        }
...
class ReadinessSummary(BaseModel):
    ...
    overall_status: ExportReadinessStatus
    total: int
    counts_by_status: dict[ExportReadinessStatus, int]


class USExportReadinessReport(BaseModel):
    """미국 선스크린 수출 준비사항 안내 리포트."""

    report_type: Literal["us_export_readiness"] = "us_export_readiness"
    result_id: str | None = None
    created_at: str
    product_name: str | None = None
...
class ReadinessInputState(str, Enum):
    """사용자가 자료 보유 상태를 어떻게 답했는지 나타낸다."""
...
class DomesticProductCategory(str, Enum):
    """국내 판매 카테고리. 미국 규제 분류를 확정하는 값은 아니다."""
...
class ReadinessSupportLevel(str, Enum):
    """현재 rule-pack이 제공하는 자동화 범위."""
...
class ReadinessEvidenceInput(BaseModel):
    """사용자가 제출한 증빙 한 항목. 입력 상태와 값은 별도로 보존한다."""

    model_config = ConfigDict(extra="allow")

    input_state: ReadinessInputState = ReadinessInputState.NOT_ENTERED
    value: Any | None = None
    evidence: list[str] = Field(default_factory=list)

    @model_validator(mode="before")
    @classmethod
    def normalize_user_state(cls, value: Any) -> Any:
        """user_state 별칭을 받고, 값만 제출된 경우 PROVIDED로 정규화한다."""
        if not isinstance(value, dict):
            return value
        value = dict(value)
        if "input_state" not in value and "user_state" in value:
            value["input_state"] = value["user_state"]
        if "input_state" not in value and value.get("value") is not None:
            value["input_state"] = ReadinessInputState.PROVIDED.value
        return value


class GenericLabelEvidence(BaseModel):
    """일반 화장품 라벨의 최소 블록별 사용자 증빙."""

    statement_of_identity: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    net_quantity: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    business_name_address: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    ingredient_declaration: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    english_required_information: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    adverse_event_contact: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)


class GenericProductEvidence(BaseModel):
    """제품별로 달라져 제조사 프로필과 분리해야 하는 증빙."""

    facility_registration: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    product_listing: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    safety_substantiation: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    color_additives: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    spf_test: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    broad_spectrum_test: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    water_resistance_test: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)
    drug_facts_label: ReadinessEvidenceInput = Field(default_factory=ReadinessEvidenceInput)


class GenericExportProduct(BaseModel):
    """generic readiness가 저장하는 제품 입력 스냅샷."""
    ...
    ingredient_amounts: dict[str, float | str] = Field(default_factory=dict)
    label_evidence: GenericLabelEvidence = Field(default_factory=GenericLabelEvidence)
    product_evidence: GenericProductEvidence = Field(default_factory=GenericProductEvidence)


class ExportReadinessRequest(BaseModel):
    """국가·제품·프로필 입력과 내부 규제 판정을 분리한 generic v2 요청."""
    ...
    def normalize_request(cls, value: Any) -> Any:
        """국가코드를 대문자로 만들고 값이 있는 프로필의 상태를 PROVIDED로 보정한다."""
        if not isinstance(value, dict):
            return value
        value = dict(value)
        country = value.get("destination_country")
        if isinstance(country, str):
            value["destination_country"] = country.strip().upper()
...
class RegulatoryRoute(BaseModel):
    ...
    code: str
    label: str
    support_level: ReadinessSupportLevel
    reasons: list[str] = Field(default_factory=list)


class AppliedRulePack(BaseModel):
    """이번 평가에 실제 적용한 rule-pack 스냅샷."""
...
class GenericReadinessItem(BaseModel):
    """사용자 증빙 상태와 내부 준비도 판정을 함께 보존하는 v2 항목."""

    id: str
    category: str
    status: ExportReadinessStatus
    user_state: ReadinessInputState
    title: str
    summary: str
    why_it_matters: str
    what_document: str
...
class ExportReadinessReport(BaseModel):
    """국가·카테고리·rule-pack 분기를 지원하는 generic readiness v2 응답."""
    ...
    profile_status: ReadinessInputState
    profile_snapshot: ExportProfile
    regulatory_route: RegulatoryRoute
    applied_rule_packs: list[AppliedRulePack]
    support_level: ReadinessSupportLevel
    summary: ReadinessSummary
    priority_actions: list[PriorityAction] = Field(default_factory=list, max_length=3)
    items: list[GenericReadinessItem]
    disclaimer: str
