...
**⚠ 아직 파이프라인에 배선하지 않았다.** 실측에서 확인된 것과 남은 문제가 있다.

확인된 것(에스코 원본 이미지 48·49번, 2026-08-19):
- 위조 2건 모두 격상 판정. 제품명·시험항목 둘 다 불일치로 정확히 읽었다.
- 제품명을 안 넘겨도 시험항목만으로 잡힌다(팀장 확정 2번 케이스가 실제로 동작).

**남은 문제 1: provider를 가려 써야 한다.**
...
**남은 문제 2: 진짜 증빙을 오판한다(배선 전 반드시 해결).**
같은 상품의 진짜 증빙(제품명 일치하는 무자극 테스트 결과지)에 대해 `claim_match=False`가
나와 위반으로 격상됐다. "피부 첩포에 의한 일차자극 시험"이 "민감성 사용적합" 주장을
뒷받침하는데도 모델이 무관하다고 봤다. 프롬프트의 claim_match 기준이 너무 엄격하다 —
시험 종류와 주장 문구의 의미 연결을 더 관대하게 잡아야 한다(자극시험 -> 무자극·민감성
적합은 정합, 자극시험 -> 미백·주름 기능성은 불일치). 이걸 안 고치고 배선하면 정직한
광고를 위반으로 찍는다.
"""
...
5. claim_match: 문서의 시험 항목이 광고가 하는 주장을 뒷받침하는가?
   예를 들어 "미백·주름개선 기능성"을 주장하는데 문서가 "피부자극 시험"이면 false다.
   못 읽으면 "unknown".
