...
def verify_citation(
    ...
    """
    if not source_id:
        return VerifyResult(False, "source_id 없음")

    by_id = {c.id: c for c in chunks}
    chunk = by_id.get(source_id.strip())
    if chunk is None:
        return VerifyResult(False, f"주입 안 된 조각 인용: {source_id}")

    if not quote or len(normalize(quote)) < _MIN_QUOTE:
        return VerifyResult(False, "quote 없음/너무 짧음")

    if normalize(quote) not in normalize(chunk.text):
        return VerifyResult(False, f"{source_id} 원문에 없는 quote")

    # span은 모델이 문장에서 집어낸 부분이다. 문장에 없으면 지어낸 것이다.
    if span and normalize(span) not in normalize(sentence):
        return VerifyResult(False, "span이 문장에 없음")
