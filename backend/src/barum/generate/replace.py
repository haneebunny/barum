...
- **항목끼리 같은 말을 반복하지 마라.** 조건표 참고 표현은 위반 유형별로 정해져
  있어서 여러 항목에 같은 값이 온다(예: `세포재생`과 `재생`에 둘 다 "피부 생기
  부여"). 그걸 그대로 받아쓰면 상세페이지에 같은 문구가 여러 번 실린다(2026-08-24
  팀장 지적: "피부 생기 부여"만 반복됐다). 참고 표현이 겹치더라도 **원문 각각의
  맥락과 어조를 살려 서로 다른 문장으로 써라.** 위 항목들에서 이미 쓴 표현은
  아래 항목에서 다시 쓰지 마라
