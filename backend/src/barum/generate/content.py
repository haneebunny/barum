"""콘텐츠 생성·개선 오케스트레이션 (FR-11/13, improve MVP).

흐름: 원본 검사(findings) → 위반 조건표 치환 → 저위험 서술 LLM 생성 → PII 제거 →
이미지 배치·가드레일 → 생성물 재검증. judge·vlm을 주입받아 유닛테스트는 오프라인.
"""
...
def generate_content(req: GenerateRequest, *, judge, vlm) -> GenerateResponse:
    """개선 모드 전체 오케스트레이션. judge·vlm 주입(테스트는 StubJudge+가짜LLM)."""
    # 1. 원본 검사 → 위반 findings
    ...
    # 6. 생성물 재검증
    combined = " ".join(s.text for s in cleaned)
