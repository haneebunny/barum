"""샘플 데이터 체험용 결과를 준비한다(유어베리 세럼, 검사·개선·신규생성 3종).

왜. 사용자가 준비된 샘플 데이터로 제품을 체험하게 한다. 매번 실시간 생성은 느리고
(요청당 ~125초) 결과 편차도 있어서, 샘플은 **한 번 준비해 커밋한 결과**를
프론트가 그대로 렌더한다(외부 런타임 의존 0). 이 스크립트가 그 결과를
만든다.
...
**외부 이미지 생성 호출을 포함한다(준비용 스크립트).** IMAGE_GENERATION_ENABLED와
무관하게 image_generator를 직접 주입해 이미지까지 준비한다. .env(GEMINI/OPENAI 키)가
...
def main() -> int:
    ...
    image_gen = get_image_generator(model=role_model("image"))  # 이미지 생성기
    judge = RagJudge(vlm)
