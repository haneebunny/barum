...
> 갱신일: 2026-08-12(D-15). 발표일: 2026-08-27(수). 기준 기획서: `2조_최종프로젝트_기획서_v1.9.docx`.
...
- 미국 프리플라이트(2단계).
- score_eval.py(base 제로샷) 43문장 재검증은 팀원A 세션 소관. **배포 파이프라인(RagJudge) 재평가는 완료**(2026-08-12): base 제로샷 60.0%/미탐1/오탐11 → RagJudge 65.0%/**미탐0**/오탐11(위반6+검토필요5). 상세 PROGRESS_BE.
...
- [팀원B] 디자이너 열린질문 2건 답변받고 리포트 화면(`barum-report.html`) 착수.
- [팀원B] tsx vs HTML 목업 진행방식 결정에 따라 화면 코딩 착수.
...
| 일 | 담당 | 비고 |
|---|---|---|
| 판정 프로그램 + 규칙집 | 팀원A | **완료.** 다음 우선순위 결정 대기 중(대기 세션) |
| 화면(리포트·콘텐츠생성) | 팀원B | 열린질문 2건 답변 후 착수 |
