...
> 갱신일: 2026-08-25(PM2 통합 QA 및 국내 검사→미국 readiness 연결 반영). 발표일: 2026-08-27(수). 기준 기획서: `2조_최종프로젝트_기획서_v1.9.docx`.
...
- **판정 백엔드는 안정화 단계이며 국내→미국 readiness 데모 연결을 진행 중이다.** 규칙집 완성 + RagJudge(규칙 우선 + VLM fallback) + RAG grounding(규정 인라인 + 실사례 벡터검색) + Supabase 이력저장까지 구현·코드리뷰·머지 완료. **FR-11/13(콘텐츠 생성, improve 모드) MVP도 완료**(`POST /generate` — 위반 조건표 치환 + LLM 저위험 서술 + PII 제거 + 이미지 가드레일 + 재검증). 현재 `GET /reports` 이력 목록과 `POST /reports/{id}/export-readiness` 국내 결과 기반 재실행 흐름을 통합 중이다.
...
### 2026-08-25 통합 QA 결과
- 백엔드 전체 테스트 **872 passed**.
- 국내 이력·이미지 캐시·미국 readiness 관련 회귀 테스트 **40 passed**.
- 프론트 TypeScript 검사와 변경 파일 lint 통과.
- 전체 프론트 lint는 기존 코드의 React effect 규칙 및 JSX 문자열 오류 14건으로 실패. 현재 통합 변경 파일의 오류는 확인되지 않음.
- production build는 코드 컴파일 오류가 아니라 Google Fonts fetch 실패로 중단됨. 발표 전 네트워크가 없는 환경을 고려해 폰트 로컬화 또는 빌드 환경 네트워크 확인이 필요하다.
