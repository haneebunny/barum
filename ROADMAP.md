...
> 갱신일: 2026-08-12(D-15). 발표일: 2026-08-27(수). 기준 기획서: v1.9(구두 확정 diff, 파일로 저장된 적 없음 — Downloads 최신 실제 파일은 `2조_최종프로젝트_기획서_v1.8.docx`, FR 정의는 이 파일이 정확함).
...
### 프론트/디자인: 진행 중
- 홈(`barum.html`)·검사(`barum-inspect.html`)·**결과 리포트(`barum-report.html`)·마이페이지(`barum-mypage.html`, FR-15) 전부 완성·머지됨.** 열린 질문 2건(지적카드 액션 인터랙션, tsx vs HTML)도 하니가 답변해서 해결됨.
- **콘텐츠 생성 화면만 착수 전** (2026-08-12 밤 착수 지시함, 디디 담당).

### 안 한 것 (다음 우선순위, 2026-08-12 밤 하니 결정: FR-11/13)
- **FR-11/13(콘텐츠 생성, 이미지 포함) → 확정된 다음 백엔드 우선순위.** FR-14(수정 권고안 생성)는 완료·머지됨(PR #31).
- 미국 프리플라이트(2단계): FR-11/13 다음으로 후순위.
...
### 이번 주 (~8/15)
- **다음 백엔드 우선순위 확정: FR-11/13(콘텐츠 생성).** 백엔드2·디디 양쪽에 착수 지시 전달됨(2026-08-12 밤).
- [팀원A/디디] 콘텐츠 생성 화면(파일명 미정) 착수. 백엔드와 API 계약 먼저 맞춘다.

### 2주차 (8/18~22) · 통합
- 화면 ↔ 판정 프로그램 연결(리포트·마이페이지가 실제 `CheckReport`/이력 API 소비).
- 콘텐츠 생성 화면(FR-11/13) 프론트·백엔드 구현.
...
| 일 | 담당 | 비고 |
|---|---|---|
| 판정 프로그램 + 규칙집 | 팀원B | **완료.** 다음 우선순위 결정 대기 중(대기 세션) |
