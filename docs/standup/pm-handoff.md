...
1. **PR `docs/fix-codename-swap` 머지.** 오늘 오후 팀원A/B 코드네임을 잘못 매핑해서 ROADMAP·PROGRESS_BE에 커밋해버린 걸 정정하는 PR. 링크는 push 시 터미널에 뜬 걸 재사용하거나 `git worktree`로 재확인.
2. **`docs/be-fr14-tier-gating` 브랜치에 새 PR 열어서 머지 필요.** 이 브랜치는 예전에 PR #28로 한 번 머지됐는데, 그 뒤 PM(나)이 이 브랜치에 커밋 3개(`b360c9a`·`38fb56c`·`a137ad0`)를 **더** 푸시했다. #28은 이미 닫혀서 그 추가 커밋들이 main에 안 들어갔다. **이 커밋들에 대수용 "작업D"(레퍼런스팩 반영 4건: 제목명 판단대상 코드확인·함량기준·피지감소 동의어 코드·모공수축) 지시가 들어있는데, 지금 main의 가이드북 HTML엔 이 내용이 없다.** 새로 PR 열어서 머지할 것. (참고: D-4 모공수축은 비비가 레퍼런스팩 쪽은 이미 반영함. D-1 제목명·D-2 함량기준도 비비가 레퍼런스팩 문서 쪽은 반영했지만, **코드 쪽 확인/구현은 대수 몫으로 아직 안 됨.** D-3 피지감소 동의어는 레퍼런스팩·코드 둘 다 아직 안 됨.)
...
4. **PR #30(`feature/be-score-eval-recheck`)은 GitHub에서 Close만 하면 됨(머지 금지).** 대수가 옛 안내(스킵 대상인 작업A) 기준으로 독자 진행한 낡은 브랜치라, 머지하면 이미 완료된 FR-14 구현(`remediation.py` 등)이 통째로 삭제된다. 새 내용 없음, 그냥 닫으면 끝.
