...
export const ReportListItemSchema = z.object({
  ...
  created_at: z.string(),
  region: RegionSchema,
  report_kind: z.enum(["domestic_check", "us_preflight"]),
  status: z.enum(["review", "done"]),
  product_name: z.string().nullable().optional(),
  n_findings: z.number().int().nonnegative().default(0),
  n_violation: z.number().int().nonnegative().default(0),
  n_needs_review: z.number().int().nonnegative().default(0),
  n_unjudged: z.number().int().nonnegative().default(0),
