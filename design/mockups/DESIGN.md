...
- **심각도/상태 표현:**
  - 색은 "지금 급한 것"에만. **빨강 = 위반·검토 필요.** 통과·안전·합격은 **색으로 신호하지 않는다**(글자·모양·아이콘으로). 나머지 상태(작성중·완료 등) = 회색.
  - **그린은 브랜드·터미널 스킨·액션 전용**(로고·태그·버튼·로그 프로세스 `[ok]/100%`). **심각도/합격 신호로 그린 금지**(CVD). "검수 완료"도 회색+체크아이콘.
...
- **입력 계약 (백엔드와 일치):** 검사 대상 = **광고 문구(text) 그리고/또는 이미지, 최소 하나.** 문구 붙여넣기를 **일급 입력**으로(드롭존과 공존, 배타 아님). 제품 정보·참고자료(전성분·함량 등)는 광고 자료와 별도 섹션으로 분리해 판정 대상이 아님을 구분한다(선택 입력).
  > 정정(2026-08-11, 기획서 v1.8 리뷰 반영): "검수 지시"(AI 자유명령 입력, mono `>`)를 광고 문구와 시각 분리해 함께 두던 구조를 뺐다. 기획서에 정의되지 않은 입력 단계였다. 같은 이유로 OCR 관련 UI·로그 문구도 뺐다(FR-4: 멀티모달이 이미지를 네이티브로 읽어 OCR은 사용자에게 드러날 개념이 아님). 되돌리지 말 것.
- **리포트 하이라이트 2모드:** 이미지 입력 → 원문 이미지 위 번호 하이라이트 / 문구 입력 → **붙여넣은 텍스트에 빨간 밑줄+번호.** 리포트가 둘 다 처리.
- **리포트 필드 = 백엔드 `CheckReport`와 1:1:** `span`(문구 위치) · 위반유형(**1호 의약품 오인 / 2호 기능성 오인 / 5호 거짓·과장·기만**, 5호는 개정법 기준·현행 4호) · 근거 조항 · 위험도 · 설명. 지적 카드가 이 필드에 매핑되게.
...
### 4.1 채움(fill) 액션 버튼의 브랜드색 — 라이트/다크 다른 토큰을 써야 한다

**리포트 화면 수용/제외 버튼**(`.fabtn`, `barum-report.html`) 같은 소형 버튼(11.5px, 대형 텍스트 기준 미달)에서 `background: var(--brand)` + `color: var(--on-brand)`를 그대로 쓰면 **라이트에서 AA 미달**이다. 검증기(§3.1 `contrast.mjs`) 실측:

- 라이트 `#fff on #0e9f6e`(=`--on-brand` on `--brand`) = **3.39:1, FAIL**(대형 텍스트 기준 3:1만 넘음, 이 버튼 크기는 대형 아님)
- 라이트 `#fff on #0f5132`(=`--on-brand` on `--brand-deep`) = **9.36:1, PASS**
- 다크 `#eaf5ee on #196340`(=`--on-brand` on `--brand`) = **6.48:1, PASS** (§4 v2 기록과 동일, 재확인됨)

**규칙: 텍스트를 얹는 소형 채움 버튼은 라이트에서 `--brand`가 아니라 `--brand-deep`을 배경으로 쓴다.** 다크는 `--brand` 그대로 둔다(다크에서 `--brand`는 이미 "딥 그린" 방향으로 설계돼 있어 그대로 AA 통과, §1 다크 v2 개정 이유 참고). `--brand-deep`은 이미 "인버스 배경(활성 nav·마크)" 용도로 문서화된 기존 토큰이라 새 색이 아니다.

이 판단은 심각도 신호가 아니라 §1의 "그린은 브랜드·**액션**·버튼 전용" 규정 범위 안의 사용이라 §0/§1과 충돌하지 않는다(2026-08-20, PM8 요청으로 디디가 검증. 라벨도 "합격/통과"가 아니라 "수용"이라 판정 결과 신호로 안 읽힌다).
...
- **로고(워드마크) = ZEN SERIF "바름"을 벡터 아웃라인 SVG로** (`barum-logo.svg`, 상단바엔 인라인). `currentColor`로 다크/라이트 자동 대응, `paint-order:stroke`+`stroke-width` 로 살짝 두껍게. **ZEN SERIF TTF는 재배포 금지 → git 제외(로컬 프리뷰 전용, `fonts/` gitignore).** 아웃라인 SVG는 폰트가 아니라 아트워크(BI/CI 사용 허용)라 커밋 OK. 파비콘·마크 이미지는 별도 제작 예정.
