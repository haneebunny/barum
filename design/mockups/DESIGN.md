...
- **입력 계약 (백엔드와 일치):** 검수 대상 = **문구(text) 그리고/또는 이미지, 최소 하나.** 문구 붙여넣기를 **일급 입력**으로(드롭존과 공존, 배타 아님). **"검수 지시"(AI 명령, mono `>`)와 "광고 문구"(검수 대상, Pretendard textarea)를 시각적으로 분리** (사용자가 광고를 지시칸에 치지 않게). 입력이 문구만이면 **verify 로그에서 OCR 단계 스킵**.
